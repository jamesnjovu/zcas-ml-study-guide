    ...
    sections: [
      {
        ...
        questions: [
          {
            id: 'q1a',
            question: "For a product recommendation system, identify the type of machine learning algorithms you would use. Explain your reasoning at length.",
...
- Association Rule Learning (Market Basket Analysis) for "frequently bought together"`,
          ...
          {
            id: 'q1b',
            question: "Identify, explain and justify the features you would extract from product recommendations to train the machine learning model.",
...
- Normalization for numerical features`,
          ...
          {
            id: 'q1c',
            question: "Discuss how you would evaluate the performance of the machine learning model during the training phase.",
...
The validation phase is critical to ensure the model will perform well on unseen data and provide valuable recommendations in production.`,
          ...
          {
            id: 'q1d',
            question: "Identify and evaluate ethical considerations when using a machine learning model to recommend products.",
...
import { BookOpen, ChevronRight, ChevronLeft, Home, FileText, CheckCircle, XCircle, RotateCcw } from 'lucide-react';
    ...
    sections: [
      {
        ...
        questions: [
          {
            id: 'q1a-2024',
            question: "Compare the performance metrics (confusion matrix, accuracy score, and classification report) generated by the SVM model. Which metric is the most informative for assessing the model's effectiveness, and why?",
...
**Detailed Analysis:**
- **Macro avg (0.37, 0.50, 0.43)**: Average across classes shows poor overall performance
- **Weighted avg (0.55, 0.74, 0.64)**: Weighted by support, shows model bias toward majority class

**Most Informative Metric: F1-Score (especially for Class 1)**

**Justification:**

**Why F1-Score is Most Informative:**
...
3. **Business Context Relevance**
   ...
   - F1-score of 0.00 for Class 1 shows complete failure

**Why Other Metrics Fall Short:**

**Accuracy (Least Informative):**
- Misleading with imbalanced data
- Can be high while model fails at primary objective
- Doesnt distinguish between class-specific performance

**Confusion Matrix (Moderately Informative):**
...
- Need both to understand trade-offs
- F1-score combines them optimally

**Comprehensive Evaluation Approach:**

**For Binary Classification with Imbalance:**
1. **Primary**: F1-score for minority class
2. **Secondary**: Precision and Recall breakdown
3. **Tertiary**: Confusion matrix for detailed analysis
4. **Last**: Overall accuracy
...
**Specific Model Issues Revealed:**

The metrics show several problems:
1. **Complete Class 1 failure** - Model needs retraining
2. **Class imbalance** - Need techniques like:
   - SMOTE (oversampling minority class)
   - Class weights adjustment
   - Different sampling strategies
3. **Feature engineering** - Current features insufficient
4. **Hyperparameter tuning** - C and kernel parameters need optimization

**Conclusion:**
The F1-score, particularly for the minority class (Class 1 = large purchases), is most informative because it:
- Reveals true predictive capability
- Isnt deceived by class imbalance
- Combines precision and recall meaningfully
- Directly relates to business objectives

The models F1-score of 0.00 for Class 1 makes it clear this model is unsuitable for production and requires significant improvement.`,
            marks: 10
          },
          {
            id: 'q1b-2024',
            question: "Critically study the results produced by the box plot graphs and interpret the results in the context of the given model.",
...
The models poor performance (F1=0.00 for Class 1) is directly explained by these distributional characteristics. Without addressing class imbalance and adding more discriminative features, the model will continue to fail at identifying large purchasers.`,
          ...
          {
            id: 'q1c-2024',
            question: "Explain how the encoding of categorical variables impact the SVM model's ability to classify large_purchase accurately and identify any potential limitations or biases introduced during this step.",
...
The current encoding contributes to the model's poor performance by failing to capture the true relationship between membership type and purchase behavior.`,
          ...
          {
            id: 'q1d-2024',
            question: "Based on the analysis of the prediction results, propose a strategy to improve the model's performance and suggest additional features or preprocessing steps that could enhance its predictive accuracy.",
...
const PastPaperRevision = () => {
  ...
  const handleToggleAnswer = () => {
    ...
    if (!showAnswer && selectedSection) {
      const questionId = selectedSection.questions[currentQuestionIndex].id;
      setCompletedQuestions(new Set([...completedQuestions, questionId]));
    }
  };
  ...
  const isQuestionCompleted = currentQuestion && completedQuestions.has(currentQuestion.id);
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600">
              ZCAS University - Machine Learning Techniques (CAS4192)
            </p>
            <p className="text-sm text-gray-500 mt-2">
              Practice with real exam questions and detailed model answers
            </p>
          </div>
          ...
          <div className="grid gap-6">
            {pastPapers.map((paper) => (
              <div
                ...
                className="bg-white rounded-xl shadow-lg p-6 cursor-pointer hover:shadow-xl transition-all border-2 border-gray-200 hover:border-indigo-400"
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <h3 className="text-2xl font-bold text-gray-800 mb-2">
                      Final Semester Examination {paper.year}
                    </h3>
                    <p className="text-gray-600">
                      {paper.sections.length} Section{paper.sections.length > 1 ? 's' : ''} • {' '}
                      {paper.sections.reduce((total, section) => total + section.questions.length, 0)} Questions
                    </p>
                  </div>
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 mb-6">
            ...
            <p className="text-gray-600">{selectedPaper.date}</p>
          </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-gray-200 rounded-full h-2">
                        <div
                          ...
                          style={{
                            width: `${(section.questions.filter(q => completedQuestions.has(q.id)).length / section.questions.length) * 100}%`
                          }}
                        />
                      </div>
                      <span className="text-sm text-gray-600">
                        {section.questions.filter(q => completedQuestions.has(q.id)).length}/{section.questions.length}
                      </span>
                    </div>
                  </div>
                  <ChevronRight className="w-8 h-8 text-indigo-600" />
...
- Misleading with imbalanced data
- Can be high while model fails at primary objective
- Doesn't distinguish between class-specific performance
...
The F1-score, particularly for the minority class (Class 1 = large purchases), is most informative because it:
- Reveals true predictive capability
- Isn't deceived by class imbalance
- Combines precision and recall meaningfully
- Directly relates to business objectives

The model's F1-score of 0.00 for Class 1 makes it clear this model is unsuitable for production and requires significant improvement.`,
            marks: 10
          },
          {
            id: 'q1b-2024',
...
The model's poor performance (F1=0.00 for Class 1) is directly explained by these distributional characteristics. Without addressing class imbalance and adding more discriminative features, the model will continue to fail at identifying large purchasers.`,
            marks: 10
          },
          {
            id: 'q1c-2024',
